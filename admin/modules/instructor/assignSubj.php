		<div class="well">
			<h3 class="fw-bold mb-3">List of Subject</h3>
				  		...
				  		function loadresult(){
							...
							foreach ($cur as $result) {
								...
								if (isset($result->CLASS_ID)){
									...
									$added = "Added";
									//for the selector
									$select = '<td width="15%"><input class="form-check-input"type="checkbox" name="selector[]" id="selector[]"  disabled CHECKED="CHECKED"  value=""/>
						  				 ' . $result->SUBJ_CODE.'</td>';
								}else{
									//status
									$added = "None";
									//for the selector
									$select ='<td width="15%"><input class="form-check-input"type="checkbox" name="selector[]" id="selector[]" value="'.$result->SUBJ_ID. '"/>
						  				 ' . $result->SUBJ_CODE.'</td>';
								}
						  		echo '<tr>';
				...
				<div class="btn-group mt-2">
				  <a href="index.php?view=instSubj&instructorId=<?php echo (isset($instructorId)) ? $instructorId : 'ID' ;?>" class="btn btn-primary"><span class="glyphicon glyphicon-back"></span>Back</a>
				  <button type="submit" class="btn btn-primary" name="Add"><span class="glyphicon glyphicon-plus-sign"></span>Assign Selected</button>
				</div>
				</form>
	  	</div><!--End of well-->
