		  	 ...
		  	 if (isset($_GET['studentId'])){
				if ($_GET['studentId']==""){
					...
					if ($NumberofResult == 0){
						...
						$button ='<a href = "index.php?view=assign&studentId='.$_GET['studentId'].'&SY='.$sy->AY.'&cid='.$sy->COURSE_ID.'&sy='.$_GET['sy'].'" class="btn btn-primary"><span class="glyphicon glyphicon-plus-sign"></span>Assign Subject</a>
		  ...
		  <!-- <form class="form-horizontal span4" action="#.php" method="POST"> -->
					<div class="panel panel-primary">
					  <div class="panel-heading">
					    <h3 class="panel-title mb-3 fw-bold"> Enrolled Subjects </h3>
					  </div>
			           ...
			           <div class="container">
					    ...
					    <span id="printout">
				           <table > 
						     ...
						     <tbody>
						     	<tr>
									...
									<b>Course/Year : </b><?php echo (isset($studcourse)) ? $studcourse->COURSE_DESC : 'Department' ;?> </br>
					     ...
					     <form class="form-horizontal span4" action="controller.php?action=delsubj&studentId=<?php echo $_GET['studentId']; ?>&cid=<?php echo $_GET['cid']; ?>&sy=<?php echo $_GET['sy']; ?>" method="POST">					    
								<table id="single" class="table table-striped table-bordered mt-0" cellspacing="1" id="table">
								  	...
								  	<tr >
								  			...
								  			<?php
												if ($_SESSION['ACCOUNT_TYPE'] == 'Administrator') {
													echo '<th width="10%" align="left"><input class="form-check-input" type="checkbox" name="chkall" id="chkall" onclick="return checkall(\'selector[]\');">Subject</th>';
												} else {
													echo '<th>Subject</th>';
												}
												?>
								  	...
								  	<?php
							 			$cid = (isset($studcourse)) ? $studcourse->COURSE_ID : 0;
											...
											foreach ($cur as $result) {
										  		...
										  		echo '<input class="form-check-input" type="checkbox" name="selector[]" id="selector[]" value="'.$result->GRADE_ID. '"/>';
						...
						<div class="btn-group mt-2" id="divButtons" name="divButtons">
						<a href="index.php?view=view&studentId=<?php echo $_GET['studentId'];?>" class="btn btn-primary">  Back</a>
						<?php 
						 if($_SESSION['ACCOUNT_TYPE']=='Administrator'){
						...
						<input type="button" value="Print" onclick="tablePrint();" class="btn btn-primary">
			 				<!-- <a href = "assignstudentsubjects.php?studentId=<?php // echo (isset($_GET['studentId'])) ? $_GET['studentId'] : 'ID' ; ?>" class="btn btn-primary"><span class="glyphicon glyphicon-plus-sign"></span>Assign Subject</a> -->
					  <!--  <button type="submit" class="btn btn-primary" name="delete"><span class="glyphicon glyphicon-trash"></span> Delete Selected</button> -->
					</form>
						</body>
						</html>		
